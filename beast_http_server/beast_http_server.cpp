// beast_http_server.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
private:
    ...
    void create_post_response() {
        if (request_.target() == "/email") {
            ...
            Json::Reader reader;
            Json::Value root;      //用于发送给客户端
            Json::Value src_root;  //用于解析客户端发送过来的json
...
void http_server(tcp::acceptor& accepter, tcp::socket& socket) {
    accepter.async_accept(socket,
        [&](beast::error_code ec) {
            if (!ec) {
                std::make_shared<http_connect>(std::move(socket))->start();
            }
            http_server(accepter, socket);
        });
